# jh-zsh-theme: personal Powerlevel10k config.
#
# Originally generated by `p10k configure` on 2026-09-17 17:38 (host: dcserver11, shared
# $HOME with JoshHuang-VM/dcserver10; propagated identically to dcserver12 at 17:51, md5
# 54e88bcc403df6138a8ab960200b3c84 across all four).
#
# Wizard options (self-described by the original file): nerdfont-v3 + powerline, small icons,
# rainbow, unicode, 24h time, round separators, blurred heads, round tails, 2 lines, solid,
# left frame, dark-ornaments, sparse, many icons, fluent, instant_prompt=verbose.
#
# Base preset: powerlevel10k's own config/p10k-rainbow.zsh (confirmed as the closest-matching
# preset -- diffed against all 6 bundled presets, rainbow differed by only 63 lines vs 506-1859
# for the others). This copy of the base preset was then re-diffed by hand and the following
# changes re-applied on top, since the byte-exact original file wasn't directly copied here:
#   - enabled `os_icon` (left, line #1, was commented out)
#   - enabled `vi_mode` (right, was commented out)
#   - enabled `time` (right, line #1, was commented out)
#   - POWERLEVEL9K_MODE: nerdfont-complete -> nerdfont-v3
#   - MULTILINE_*_PROMPT_PREFIX color: %242F -> %240F (darker ornaments)
#   - MULTILINE_*_PROMPT_SUFFIX: cleared entirely (left-frame-only look)
#   - MULTILINE_FIRST_PROMPT_GAP_CHAR: ' ' -> '-' (horizontal rule to align line 1)
#   - MULTILINE_FIRST_PROMPT_GAP_FOREGROUND: 242 -> 240
#   - separator glyphs switched from sharp (\uE0B0-\uE0B3) to round/blurred
#     (\uE0B4-\uE0B7, plus solid-block segment caps ▓▒░ / ░▒▓)
#
# If this doesn't render pixel-identical to the original, that's expected -- re-run
# `p10k configure` once and re-diff against config/p10k-rainbow.zsh to true it up, then replace
# this file. See README.md in this repo for the full reconstruction methodology and caveats.
#
# Type `p10k configure` to generate a fresh config from scratch instead.
#
